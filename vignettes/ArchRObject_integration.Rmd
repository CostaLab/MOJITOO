...
proj <- ArchRProject(ArrowFiles = ArrowFiles, outputDirectory = "PBMC", copyArrows = T)
...
### 4.  Add PeakSets and  RNA & ATAC dimension reductions
```{r}
...
### 5. RUN MOJITOO
```{r}
proj <- loadArchRProject("PBMC")
proj <- mojitoo(
     object=proj,
     reduction.list = list("RNA_PCA", "IterativeLSI"),
     dims.list = list(1:50, 1:29), ## exclude last dimension of LSI due to rank(IterativeLSI)==29
